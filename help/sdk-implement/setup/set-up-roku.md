...
1. Experience Cloud 방문자 ID를 구성합니다.
   ...
   **Experience Cloud 방문자 ID 서비스 메서드**
   ...
   | `visitorSyncIdentifiers` | Experience Cloud 방문자 ID를 사용하면 각 방문자와 연결할 수 있는 추가 고객 ID를 설정할 수 있습니다. 방문자 API는 여러 다른 고객 ID의 범위를 구분하기 위해 동일한 방문자의 여러 고객 ID와 고객 유형 식별자를 허용합니다. 이 메서드는 에 해당합니다 `setCustomerIDs`. For example: <br/><br/>`identifiers={}` <br/>`identifiers["idType"]="idValue"` <br/>`ADBMobile().visitorSyncIdentifiers(identifiers)` |
   | `setAdvertisingIdentifier` | SDK에서 광고에 대한 RIDA(Roku ID)를 설정하는 데 사용됩니다. 예: <br/><br/> `ADBMobile().setAdvertisingIdentifier(`<br/>  Roku`"<sample_roku_identifier_for_advertising>")` SDK <br/><br/><br/>getRIDA() [API를 사용하여 RIDA(Advertising)용 Roku ID를](https://developer.roku.com/docs/references/brightscript/interfaces/ifdeviceinfo.md#getrida-as-dynamic) 가져옵니다. |
