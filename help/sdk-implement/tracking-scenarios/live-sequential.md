...
이는 하트비트 컨텐츠 시작 호출과 거의 동일하게 보이지만 &quot;s:event:type&quot; 매개 변수에 키 차이가 있습니다. 모든 매개 변수가 여기에 있어야 합니다.
...
미디어 재생 중에 10초마다(기본 컨텐츠의 경우) 또는 1초마다(광고의 경우) 하나 이상의 하트비트를 전송하는 타이머가 있습니다. 이러한 하트비트에 재생, 광고, 버퍼링 및 기타 많은 정보가 포함됩니다. 각 하트비트의 정확한 컨텐츠는 이 문서의 범위를 벗어나며, 유효성을 확인하기 위해 중요한 사항은 재생이 계속되는 동안 하트비트가 일관되게 트리거된다는 것입니다.
