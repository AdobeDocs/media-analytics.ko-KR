...
이 시나리오는 두 개의 별도 미디어에 대해 동시에 실행되는 세션이 두 개 있다는 점을 제외하고, [광고 없이 VOD 재생](/help/use-cases/tracking-scenarios/vod-no-intrs-details.md) 시나리오와 동일합니다. 이러한 각 세션은 `MediaHeartbeat`의 별도 인스턴스를 사용합니다.

명시하지 않은 경우, 네트워크 호출은 [광고 없이 VOD 재생](/help/use-cases/tracking-scenarios/vod-no-intrs-details.md) 시나리오와 동일합니다.
...
`MediaAnalyticsProvider`와 `MediaHeartbeat`의 두 인스턴스는 각각 고유한 세션 ID가 있는 두 개의 별도 세션을 추적합니다. Charles 디버깅 도구 또는 디버그 로그에서 두 세션은 세션 ID 값으로 식별할 수 있습니다. Android에서 이 시나리오를 표시하려면 다음 코드를 설정합니다.
