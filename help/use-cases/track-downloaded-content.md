---
title: Adobe 스트리밍 미디어에서 오프라인 다운로드한 컨텐츠를 추적하는 방법
...
>[!IMPORTANT]
>
>이전에 다운로드한 컨텐츠를에 보낼 수도 있습니다 `/api/v1/sessions` API. 다운로드한 컨텐츠를 추적하는 이러한 방법은 다음과 같습니다 **사용되지 않음** 그리고 **제거됨** 나중에


다음 `/api/v1/sessions` API는 세션 초기화 이벤트만 허용합니다.
새 API를 사용하는 경우 이전에 필수입니다 `media.downloaded` 플래그가 더 이상 필요하지 않습니다.
를 사용하는 것이 좋습니다 `/api/v1/downloaded` 새로 다운로드한 컨텐츠 구현을 위한 API와, 이전 API를 사용하는 기존 구현을 업데이트하는 API입니다.
