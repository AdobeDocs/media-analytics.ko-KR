---
title: 미디어 추적 타임라인의 콘텐츠 끝까지 보기에 대해 알아보기
description: 플레이헤드 타임라인과 해당 사용자의 작업을 더 자세히 알아보십시오. 각 작업과 그에 수반되는 요청에 대한 세부 사항을 알아보십시오.
...
이 호출은 비디오를 _재생하도록 사용자에게 신호_&#x200B;를 보냅니다. <br/><br/>세션 내의 모든 후속 추적 호출을 식별하는 데 사용되는 클라이언트에 세션 ID( `{sid}` )가 반환됩니다. 플레이어 상태가 아직 &quot;재생 중&quot;이 아니라 &quot;시작 중&quot;입니다. <br/><br/>[필수 세션 매개 변수](/help/media-collection-api/mc-api-ref/mc-api-sessions-req.md)는 요청 본문의 `params` 맵에 포함해야 합니다. <br/><br/>백엔드에서 이 호출은 Adobe Analytics 시작 호출을 생성합니다.
