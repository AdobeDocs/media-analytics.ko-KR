...
JSON validation schemas for each event type are publicly accessible here: `{uri}/api/v1/schemas/{eventType}` (e.g., `{uri}/api/v1/schemas/sessionEnd`). 이러한 JSON 유효성 검사 스키마는 각 이벤트 유형에 대한 올바른 요청 본문 매개 변수를 결정하기 위한 절대 기관입니다.
...
>[!NOTE]
>
>세션 컨텍스트는 컬렉션 레이어에서 사용할 수 없으므로 세션 수준 유효성 검사가 불가능합니다.
