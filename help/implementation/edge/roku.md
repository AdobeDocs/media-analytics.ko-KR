...
구성 키 및 전체 API에 대해서는 [AEP Roku SDK API 참조](https://github.com/adobe/aepsdk-roku/blob/main/Documentation/api-reference.md)를 참조하십시오.

## 미디어 이벤트 추적

세션이 열린 후 `sendMediaEvent`(으)로 각 미디어 이벤트를 보냅니다. 정확한 페이로드는 각 [이벤트](/help/implementation/events/overview.md) 및 [변수](/help/implementation/variables/overview.md) 페이지에서 **Roku** 탭을 참조하십시오.
