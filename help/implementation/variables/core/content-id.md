...
[`sendEvent`](https://experienceleague.adobe.com/kr/docs/experience-platform/collection/js/commands/sendevent/overview)을(를) 호출할 때 `xdm.mediaCollection.sessionDetails` 내에서 `name`을(를) 설정합니다.
...
`xdm.mediaCollection.sessionDetails` 내에서 `name`(콘텐츠 ID)을(를) 사용하여 [sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart) 끝점을 호출합니다.
...
콘텐츠 ID를 두 번째 인수로 `ADBMobile.media.createMediaObject`에 전달합니다.
