...
| **XDM 컬렉션 필드** | [`xdm.mediaCollection.advertisingDetails.creativeID`](https://experienceleague.adobe.com/ko/docs/experience-platform/xdm/data-types/advertising-details-collection) |
| **Audience Manager 트레이트** | `c_contextdata.a.media.ad.creative` |
| **필수** | 아니요 |
...
[`sendEvent`](https://experienceleague.adobe.com/kr/docs/experience-platform/collection/js/commands/sendevent/overview)을(를) 호출할 때 `xdm.mediaCollection.advertisingDetails` 내에서 `creativeID`을(를) 설정합니다.
...
`media.adStart`에 대해 `sendMediaEvent`을(를) 호출할 때 `xdm.mediaCollection.advertisingDetails` 내에서 `creativeID`을(를) 설정합니다.
...
`xdm.mediaCollection.advertisingDetails` 내에서 `creativeID`을(를) 사용하여 [adStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/ads/#adstart) 끝점을 호출합니다.
...
표준 광고 메타데이터 개체에서 `ADBMobile.media.AdMetadataKeys.CREATIVE_ID`을(를) 사용하여 크리에이티브 ID를 설정합니다.
...
표준 광고 메타데이터 개체에서 `MEDIA_AdMetadataKeyCREATIVE_ID`을(를) 사용하여 크리에이티브 ID를 설정합니다.
