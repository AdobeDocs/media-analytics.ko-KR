...
Chromecast에는 `PlayerState` 상수가 없으므로 `"pictureInPicture"` 문자열과 함께 `ADBMobile.media.createStateObject`을(를) 직접 사용하십시오.
