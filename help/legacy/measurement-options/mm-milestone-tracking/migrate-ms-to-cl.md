...
비디오 측정의 핵심 개념은 이정표 및 사용자 지정 링크 추적에도 동일하며, 이 추적에서는 비디오 플레이어 이벤트를 가져와서 분석 메서드에 매핑하고, 플레이어 메타데이터 및 값도 가져와서 분석 변수에 매핑합니다. 사용자 지정 링크 접근 방식은 구현과 수집된 데이터 모두를 슬림화하고 단순화하는 것으로 간주해야 합니다. 사용자 지정 링크 솔루션을 사용하면 비디오 측정에 대해 사전에 정의된 변수나 메서드가 없으므로 전체 사용자 지정 설정이 필요합니다. 시작 및 완료와 같은 기본 플레이어 이벤트에 대한 사용자 지정 링크 추적 호출을 가리키도록 플레이어 이벤트 코드를 업데이트할 수 있어야 합니다. 자세한 내용은 [사용자 지정 링크 구현 안내서](/help/legacy/measurement-options/cl-in-aa/cl-impl-guide.md)를 참조하십시오.
...
| Media.contextDataMapping | `s.Media.contextDataMapping = {` <br> `  "a.media.name":"eVar2,prop2",` <br> `  "a.media.segment":"eVar3",` <br> `  "a.contentType":"eVar1",` <br> `  "a.media.timePlayed":"event3",` <br> `  "a.media.view":"event1",` <br> `  "a.media.segmentView":"event2",` <br> `  "a.media.complete":"event7",` <br> `  "a.media.milestones": {` <br> `    25:"event4",` <br> `    50:"event5",` <br> `    75:"event6"` <br> `  }` <br> `};` | 해당 사항 없음 | 이제 처리 규칙을 통해 eVar, prop 및 이벤트에 컨텍스트 데이터를 매핑하는 작업이 완료됩니다. |
...
| 이정표 | 이정표 구문 | 사용자 지정 링크 | 사용자 지정 링크 구문 |
| --- | --- | --- | --- |
| Media.autoTrack | `s.Media.autoTrack` <br> `  = true;` | 해당 사항 없음 | 사용할 수 없음. |
| Media.autoTrackNetStreams | `s.Media.` <br> `  autoTrackNetStreams` <br> `  = true` | 해당 사항 없음 | 사용할 수 없음. |
| Media.completeByCloseOffset | `s.Media.` <br> `  completeByCloseOffset` <br> `  = true` | 해당 사항 없음 | 사용할 수 없음. |
| Media.completeCloseOffsetThreshold | `s.Media.` <br> `  completeCloseOffsetThreshold` <br> `  = 1` | 해당 사항 없음 | 사용할 수 없음. |
...
| Media.trackSeconds | `s.Media.` <br> `  trackSeconds` <br> `  = 15` | 해당 사항 없음 | 사용할 수 없음. |
| Media.trackMilestones | `s.Media.` <br> `  trackMilestones` <br> `  = "25,50,75";` | 해당 사항 없음 | 사용할 수 없음. |
| Media.trackOffsetMilestones | `s.Media.` <br> `  trackOffsetMilestones` <br> `  = "20,40,60";` | 해당 사항 없음 | 사용할 수 없음. |
| Media.segmentByMilestones | `s.Media.segmentByMilestones` <br> `  = true;` | 해당 사항 없음 | 사용할 수 없음. |
| Media.segmentByOffsetMilestones | `s.Media.` <br> `  segmentByOffsetMilestones` <br> `  = true;` | 해당 사항 없음 | 사용할 수 없음. |

### 광고 추적 변수

| 이정표 | 이정표 구문 | 사용자 지정 링크 | 사용자 지정 링크 구문 |
| --- | --- | --- | --- |
| Media.adTrackSeconds | `s.Media.` <br> `  adTrackSeconds` <br> `  = 15` | 해당 사항 없음 | 사용할 수 없음. |
| Media.adTrackMilestones | `s.Media.` <br> `  adTrackMilestones` <br> `  = "25,50,75";` | 해당 사항 없음 | 사용할 수 없음. |
| Media.adTrackOffsetMilestones | `s.Media.` <br> `  adTrackOffsetMilestones` <br> `  = "20,40,60";` | 해당 사항 없음 | 사용할 수 없음. |
| Media.adSegmentByMilestones | `s.Media.` <br> `  adSegmentByMilestones` <br> `  = true;` | 해당 사항 없음 | 사용할 수 없음. |
| Media.adSegmentByOffsetMilestones | `s.Media.` <br> `  adSegmentByOffsetMilestones` <br> `  = true;` | 해당 사항 없음 | 사용할 수 없음. |
...
| Media.openAd | `s.Media.openAd(` <br> `  name,` <br> `  length,` <br> `  playerName,` <br> `  parentName,` <br> `  parentPod,` <br> `  parentPodPosition,` <br> `  CPM)` | 해당 사항 없음 | 사용할 수 없음. |
| 이름 | `name`: (필수) 광고 이름 또는 ID입니다. | 해당 사항 없음 | 사용할 수 없음. |
| length | `length`: (필수) 광고 길이입니다. | 해당 사항 없음 | 사용할 수 없음. |
| playerName | `playerName`: (필수) 광고를 보는 데 사용되는 미디어 플레이어의 이름입니다. | 해당 사항 없음 | 사용할 수 없음. |
| parentName | `parentName`: 광고가 임베드된 기본 콘텐츠의 이름 또는 ID입니다. | 해당 사항 없음 | 사용할 수 없음. |
| parentPod | `parentPod`: 기본 콘텐츠에서 광고가 재생되는 위치입니다. | 해당 사항 없음 | 사용할 수 없음. |
| parentPodPosition | `parentPodPosition`: Pod 내에서 광고가 재생되는 위치입니다. | 해당 사항 없음 | 사용할 수 없음. |
| CPM | `CPM`: 이 재생에 적용되는 CPM 또는 암호화된 CPM(앞에 &quot;~&quot;가 붙음)입니다. | 해당 사항 없음 | 사용할 수 없음. |
| Media.click | `s.Media.click(name, offset)` | `s.tl()` | 사용자 지정 링크 분석 호출을 사용하여 클릭 수를 추적하십시오. |
| Media.close | `s.Media.close(mediaName)` | 해당 사항 없음 | 사용할 수 없음. |
...
| Media.play | `s.Media.play(` <br> `  name,` <br> `  offset,` <br> `  segmentNum,` <br> `  segment, ` <br> `  segmentLength)` | 해당 사항 없음 | 사용할 수 없음. |
| Media.stop | `s.Media.stop(` <br> `  mediaName,` <br> `  mediaOffset)` | 해당 사항 없음 | 사용할 수 없음. |
...
| Media.track | `s.Media.track(` <br> `  mediaName)` | 해당 사항 없음 | 사용할 수 없음. |
