...
>[!IMPORTANT]
>
>The documentation provided here is specific to clients utilizing version 1.5 or higher of Adobe's *Media SDK* for heartbeat measurement, or Adobe's newer *Media Collection API* for heartbeat measurement. 이전 마일스톤 비디오 구현에 대한 지침은 포함되지 않습니다. 모든 고객이 개선 사항 및 확장된 측정 기능을 활용할 수 있도록 두 개의 최신 미디어 추적 솔루션 중 하나 또는 둘 다 채택할 것을 권장합니다. 아래에서 [최신 솔루션으로 전환했을 때 이점](media-overview.md#section_cnj_5st_p1b)을 확인할 수 있습니다. Adobe는 비디오 추적 방법을 계속 지원할 것이지만 예정된 업데이트, 수정 사항 또는 기능 개선 사항은 없습니다. 추가 질문이 있는 경우 Adobe 계정 관리자에게 문의하십시오.
...
* **전체적인 사진 - 모든 컨텐츠 배포자에게 다양한 데이터 포인트를** 통합하여 모든 미디어 활동을 한눈에 파악할 수 있고 [통합 분석](federated-analytics.md) 기능을 통해 모든 채널에서 참여도를 측정하고 고객의 의견을 수렴할 수 있습니다.
...
미디어용 Adobe Analytics는 다음 두 가지 방법을 통해 측정할 수 있습니다. 레거시 마일스톤 메서드 (비디오만 해당) 와 현재 하트비트 메서드 (Media SDK와 Media Collection API 모두에 모두 포함된 오디오 및 비디오) 입니다. 하트비트 메서드는 기본 측정 메서드이므로, 모든 클라이언트가 아래에 설명된 이점을 이용할 수 있도록 이 버전으로 이동할 것을 권장합니다(이 버전이 아직 없는 경우).
...
**TLS 알림 —** Adobe는 이전 보안 프로토콜의 사용이 필요한 보안 준수 표준을 보유하고 있습니다. 끊임없이 진화하는 보안 프로토콜 표준을 충족하기 위해 Adobe는 최신 및 안전한 버전을 사용하기 위해 TLS 1.2의 사용으로 전환하고 있습니다. 2019 년 2 월 20 일부터 Adobe는 TLS 1.1 이상만 지원합니다. 이러한 변화를 통해 Adobe는 더 이상 TLS 1.0를 배포하는 이전 장치나 웹 브라우저를 사용하여 최종 사용자의 데이터를 수집하지 않습니다. TLS 1.2로 마이그레이션하면 보안이 강화됩니다. 세부 사항을 살펴보고 원활한 전환을 위한 변경을 계획하는 것이 중요합니다.

>[!NOTE]
>
>TLS는 현재 웹 브라우저 및 네트워크를 통해 안전하게 데이터를 교환해야 하는 기타 애플리케이션에서 사용되는 가장 널리 배포된 보안 프로토콜입니다.
