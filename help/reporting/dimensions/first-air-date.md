...
**첫 방송 날짜** 차원은 콘텐츠가 TV에 처음 방송된 날짜를 보고합니다. 이를 사용하여 새 릴리스에 대한 참여와 이전 콘텐츠에 대한 참여를 분리하십시오.

## 이 차원이 채워지는 방법

첫 방송 날짜는 세션 시작 시 플레이어가 설정합니다.
...
| Customer Journey Analytics | [`mediaReporting.sessionDetails.firstAirDate`](https://experienceleague.adobe.com/ko/docs/experience-platform/xdm/data-types/session-details-reporting) |
...
Adobe은 보고서 세트에 대해 **[[!UICONTROL 비디오 메타데이터]](/help/reporting/media-reports-enable.md)**&#x200B;을(를) 사용하도록 설정하면 자동으로 첫 방송 날짜 분류 구조를 만듭니다. [분류 세트](https://experienceleague.adobe.com/en/docs/analytics/components/classifications/sets/overview.html)를 사용하여 분류를 채우고 유지 관리합니다.

이 접근 방법에서는 각 콘텐츠 ID와 첫 방송 날짜 간에 보장된 1:1 관계를 제공합니다. 분류 업데이트는 해당 ID의 모든 내역 데이터에 소급하여 적용됩니다.

>[!IMPORTANT]
>
>첫 번째 방송 날짜 분류 이름을 변경하지 마십시오. 이름을 바꾸면 Adobe에서 원래 분류를 다시 만들어 복제가 발생할 수 있습니다.
