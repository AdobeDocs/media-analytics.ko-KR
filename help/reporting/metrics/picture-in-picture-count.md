...
미디어 백엔드는 모든 PIP(Picture-in-Picture) 상태 시작 이벤트에서 `mediaReporting.states[]`의 `pictureInPicture` 항목의 `count` 필드를 증가시킵니다. 지표는 닫기 호출에 보고됩니다.

| 보고 시스템 | 소스 |
| --- | --- |
| Adobe Analytics | [[!UICONTROL 플레이어 상태 추적]](/help/reporting/media-reports-enable.md)이(가) 활성화되면 컨텍스트 데이터 `a.media.states.pictureinpicture.count`에서 자동으로 수집됩니다. |
| Customer Journey Analytics | [`mediaReporting.states[]`](https://experienceleague.adobe.com/ko/docs/experience-platform/xdm/data-types/media-reporting-details) 항목 위치: `name = "pictureInPicture"`, 필드: `count` |
