...
미디어 백엔드는 모든 캡션 사용 상태 시작 이벤트에서 `mediaReporting.states[]`의 `closedCaptioning` 항목에 있는 `count` 필드를 증가시킵니다. 지표는 닫기 호출에 보고됩니다.
...
| Customer Journey Analytics | [`mediaReporting.states[]`](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/data-types/media-reporting-details) 항목 위치: `name = "closedCaptioning"`, 필드: `count` |
