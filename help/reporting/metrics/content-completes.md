...
`media.sessionComplete` 이벤트가 수신되면 미디어 백엔드가 `mediaReporting.sessionDetails.isCompleted = true`을(를) 설정합니다. 지표는 닫기 호출에 보고됩니다. 명시적 `sessionComplete` 없이 시간 초과된 세션은 완료로 계산되지 않습니다.
...
| Customer Journey Analytics | [`mediaReporting.sessionDetails.isCompleted`](https://experienceleague.adobe.com/ko/docs/experience-platform/xdm/data-types/session-details-reporting) |
