...
미디어 백엔드는 기본 콘텐츠에 대한 하나 이상의 `media.play` 이벤트가 수신된 세그먼트를 포함하는 모든 닫기 호출에 대해 `mediaReporting.sessionDetails.hasSegmentView = true`을(를) 설정합니다. 지표는 닫기 호출에 보고됩니다. Media Edge API 경로에서 세그먼트 보기는 컨텐츠가 시작될 때와 동일한 조건에서 실행됩니다. 둘 다 기본 콘텐츠에 `media.play`이(가) 필요합니다.
