...
미디어 백엔드는 `statesStart`에 `closedCaptioning`이(가) 있는 `media.statesUpdate` 이벤트가 처음 수신될 때 `closedCaptioning` 항목에 대한 `mediaReporting.states[]`의 `isSet` 플래그를 `true`(으)로 설정합니다. 지표는 닫기 호출에 보고됩니다.
...
| Customer Journey Analytics | [`mediaReporting.states[]`](https://experienceleague.adobe.com/ko/docs/experience-platform/xdm/data-types/media-reporting-details) 항목 위치: `name = "closedCaptioning"`, 필드: `isSet` |
